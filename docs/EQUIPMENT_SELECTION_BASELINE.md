...
本阶段完成的是“回路职责 + 额定值等级 + 1.10s热效应预筛选”，不是厂家精确型号定型。系统运行、T10/SVG、海拔、污秽和保护时间已按课程设计口径冻结；凡仍缺少41℃修正、精确变流器模型、标准完整短路计算或厂家页级证据的项目，最终状态保持 `pending`。
...
| 10kV T10进线、母联开关柜 | 12kV、2000A、31.5kA、80kA峰值、31.5kA/4s | 目标等级；厂家家族可得性已登记 | 数值与1.10s I²t预筛通过，精确柜型和41℃适配待定 |
...
1. 41℃条件下开关柜、母线和LGJ-400/50的现行厂家热额定值；LGJ当前仅按475.242A课程保守参考闭环；
2. 220kV母联精确潮流及第三回线路远期运行方式；
3. 按GB/T 15544.1-2023复核R/X、电压系数、触头分离电流与峰值；
4. 新能源变流器、SVG和必要的电动机反馈及衰减模型；
5. 厂家精确型号、正式样本版本、页码、41℃服务条件、d级外绝缘和外形安装图；
