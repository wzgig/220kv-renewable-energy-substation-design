...
本项目使用 AutoCAD Core Console、GB A0-A4 图框、DWG To PDF 打印配置和黑白打印样式。Core Console 已实测完成 DXF 原生 AUDIT、2018 DWG 保存和 A1 PDF 打印，并以 PDF/PNG 渲染目检完成闭环。
...
4. 使用项目 PowerShell 脚本调用 AutoCAD Core Console，执行 AUDIT、保存 2018 DWG 并通过官方 PC3/CTB 打印。
5. 用 Poppler 将 PDF 渲染为 PNG，目检文字、线宽、图框、比例、标注和打印范围。

Python 依赖统一写入根目录 `requirements.txt` 并安装到项目虚拟环境，不污染系统环境：ezdxf 用于 DXF 生成和语义审计，pypdf 用于规范化 AutoCAD PDF。
...
drawings/source/switchyard_line_bay_detail_a1.dxf
...
drawings/exports/switchyard_line_bay_detail_a1.pdf
drawings/exports/switchyard_line_bay_detail_a1.png
~~~

中文文字采用本机已验证的 `txt.shx + gbcbig.shx` 大字体组合，避免 Core Console 对 TTC 字体替换成问号。DXF 使用 R2018/AC1032、毫米单位、A1 横向 841×594 模型空间纸面坐标；DWG 作为本地 AutoCAD 兼容输出，不进入公开 Git 历史。

三张必交图已有AutoCAD Core Console原生AUDIT与目检PASS证据，SEC-220-L1-01的DXF/PDF/PNG也已生成。四图集成后必须统一重跑AUDIT、A1单页PDF、150dpi PNG、逐图目检和语义检查；PDF规范化还须清除AutoCAD SHX文字批注层并确认`/Annots=0`，且规范化前后PNG像素哈希一致。公开仓库使用DXF/PDF/PNG，本地DWG不进入Git。

## 制图顺序

### 1. 电气主接线简图

- 输入：最终主接线方案、主变/T10/所用变联结组与阻抗、回路/母线/母联、三相CT/PT、零序CT、MOA、接地变+低电阻和接地开关配置。
- 重点：220、35、10、0.4kV 电压层次清晰；本期与预留回路区分；设备编号唯一；220kV ES接在线路侧QS之后；35/10kV接地设备包容量与母联接地源联锁说明齐全。
- 验收：图中回路数与任务书、回路表、设备表完全一致；7回35kV架空入口MOA、5回35kV电缆ZCT、10kV电缆/SVG ZCT及“母联合闸前仅保留健康侧1套接地源”均可在图面或说明中定位。
...
### 4. SEC-220-L1-01 线路间隔增强详图

- 图幅与比例：A1横向、1:50。
- 设备链：220kV I段管形母线→母线侧QS→QF→TA→线路侧QS+常开ES→CVT→YH10W-204/532 MOA→出线构架。
- 表达内容：设备编号、课程额定参数、基础、构架与设备接地、地下接地网接口、主要标高、水平尺寸及A1/A2/C类净距。
- 边界：设备外形、基础荷载和端子方向由厂家图覆盖；缺少土壤电阻率和接地网几何时，不填写接地网电阻、接触电压、跨步电压，也不声称避雷针已覆盖全站。
...
- SEC-220-L1-01 采用A1/1:50放大线路间隔，净距、标高和设备编号必须与LAY-220-01、SEC-220-01同源；
- 设备外形、道路、建筑和基础是课程几何假设，精确厂家图优先覆盖。

## 图形标准草案

- 当前中文文字使用 AutoCAD 自带 `txt.shx + gbcbig.shx`，西文与数字使用兼容字体，并以最终 PDF/PNG 打印效果为准。
- 不提交版权不明的第三方 SHX；避免外部参照和绝对路径。
- 图层至少区分：母线、一次设备、导线、符号、文字、尺寸、中心线、构筑物、道路、预留和图框。
- 黑白打印依赖线宽表达，不以屏幕颜色代替最终线宽。
- 原生文件保存为 2018 格式，并输出 PDF 与 PNG；如教师或提交平台要求，再另存 2013 兼容版。
...
- 当前候选树以公开 DXF/数据/脚本重建关键图形，本地 DWG 不再进入新提交；既有历史的旧 DWG 待用户确认后清理。
