...
> [!IMPORTANT]
> 当前公开树及 `main` 可达历史只保留原创且已脱敏的成果。任务书原件、课程讲义、教材手册、他人设计书、参考 DWG、手稿照片和带个人元数据的文件均保留在本地私有材料区；旧 DWG 与身份记录已通过历史重写从 `main` 可达历史移除，三份可编辑 DWG 仅在本地保留。
...
| 仓库发布 | 当前树隐私扫描、51 项测试、64 项健康检查、公开历史重写、精确租约强推及 GitHub Actions 均已通过 | 后续有效修改继续执行提交、推送、Actions 和远端一致性闭环 |
